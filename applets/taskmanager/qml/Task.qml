...
import plasma.applet.macesque.taskmanager as TaskManagerApplet
...
PlasmaCore.ToolTipArea {
    ...
    // "Hide minimized tasks" option; windows demanding attention stay
    // visible so they can still summon the user.
    readonly property bool effectivelyHidden: !inPopup
        && Plasmoid.configuration.hideMinimizedWindows
        && model.IsMinimized
        && !model.IsDemandingAttention

    visible: !effectivelyHidden
    ...
    // Whether this task renders a window thumbnail instead of its icon,
    // per the thumbnailMode setting (see main.xml for the value mapping).
    readonly property bool showThumbnail: {
        if (!model.IsWindow) {
            return false;
        }

        switch (Plasmoid.configuration.thumbnailMode) {
        case 1: // AllWindows
            return true;
        case 2: // Unfocused (any window that is not the active one)
            return !model.IsActive;
        case 3: // Windows on other virtual desktops
            return !model.IsOnAllVirtualDesktops
                && !model.VirtualDesktops.includes(tasksRoot.currentVirtualDesktop);
        default: // MinimizedOnly
    ...
    onSmartLauncherEnabledChanged: {
        if (smartLauncherEnabled && !smartLauncherItem) {
            const component = Qt.createComponent("plasma.applet.macesque.taskmanager", "SmartLauncherItem");
    ...
    KSvg.FrameSvgItem {
        ...
        DragHandler {
            ...
            onActiveChanged: {
                if (active) {
                    delegateThumb.grabToImage(result => {
    ...
    // Dock-style window thumbnail, shown instead of the icon per the
    // thumbnailMode setting. Fills the whole task button; TaskImage derives
    // the icon and thumbnail rects from the frame margins so that the
    // thumbnail rect matches the geometry KWin minimize effects compute
    // from the published iconGeometry.
    Loader {
        id: delegateThumb
        ...
        sourceComponent: TaskImage {
            highlighted: task.highlighted
            decoration: model.decoration
            winIdList: model.WinIdList
            ...
            frameMarginLeft: taskFrame.margins.left
            frameMarginRight: taskFrame.margins.right
            frameMarginTop: taskFrame.margins.top
            frameMarginBottom: taskFrame.margins.bottom
        }
    }
