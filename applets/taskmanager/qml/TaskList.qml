...
GridLayout {
    ...
    // Tasks hidden by e.g. the "hide minimized tasks" option must not
    // occupy layout space or skew the fill calculations.
    readonly property int visibleCount: children.filter(child => child.visible).length
