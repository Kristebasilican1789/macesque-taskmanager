...
PlasmoidItem {
    ...
    // This applet always renders icon-only (dock-style) task buttons; text
    // labels appear only inside group popups.
    readonly property bool iconsOnly: true

    // Exposed so task delegates can decide thumbnail eligibility per window.
    readonly property string currentVirtualDesktop: virtualDesktopInfo.currentDesktop
...
//END TODO
    ...
    function publishIconGeometries(taskItems: /*list<Item>*/var): void {
        ...
        for (let i = 0; i < taskItems.length - 1; ++i) {
            ...
            if (!task.visible || !task.model.IsLauncher && !task.model.IsStartup) {
    ...
    readonly property TaskManager.TasksModel tasksModel: TaskManager.TasksModel {
        ...
        hideActivatedLaunchers: Plasmoid.configuration.hideLauncherOnStart
        sortMode: sortModeEnumValue(Plasmoid.configuration.sortingStrategy)
        // Fixed dock semantics, matching upstream's icons-only task manager:
        // launchers are always kept separate and pinned in place, so pinned
        // apps absorb their windows instead of spawning detached instances.
        launchInPlace: true
        separateLaunchers: true

        groupMode: groupModeEnumValue(Plasmoid.configuration.groupingStrategy)
        // Groups always expand into a popup; inline groups would only make
        // sense with stacked thumbnails, which is not implemented.
    ...
    Item {
        ...
        TriangleMouseFilter {
            ...
            TaskList {
                ...
                readonly property real widthOccupation: visibleCount / columns
                readonly property real heightOccupation: visibleCount / rows
